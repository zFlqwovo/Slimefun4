    ...
    public void onExecute(@Nonnull CommandSender sender, @Nonnull String[] args) {
        if (sender.hasPermission("slimefun.command.cleardata") || sender instanceof ConsoleCommandSender) {
            if (args.length >= 3) {
                ...
                List<String> availableClearTypes = List.of(
                    "block",
                    "oil"
                );
                        ...
                        for(String cleartype : clearTypes){
                            if(cleartype.equals("block")){
                                ...
                                for(GEOResource resource : Slimefun.getRegistry().getGEOResources().values()){
                                    if(resource.getKey().toString().equals(new NamespacedKey(Slimefun.instance(), "oil"))){
                                        oil = resource;
                                    }
                                }
                                Set<SlimefunChunkData> datas = controller.getAllChunkDatas(world);
                                GEOResource finalOil = oil;
                                datas.forEach(x -> x.removeData(finalOil.getKey().toString().replace(":", "-")));
                            }
                        }
                    }
                }
